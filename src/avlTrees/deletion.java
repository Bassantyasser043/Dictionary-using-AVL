...
public class deletion {
	...
	node Deletion(node no,String elem) {
		if(no == null) {
			return no; //root= null(already empty tree)
		}

		if(no.element.compareTo(elem)>0) {
			no.left=Deletion(no.left,elem);
			//if(no.left ==null) {
			//	return no.left;
			//}
		}else if(no.element.compareTo(elem)<0) {
	...
	public int size(node nerd)
	{
		if (nerd == null)
			return 0;
		else
			return(size(nerd.left) + 1 + size(nerd.right));
	}
